...
"@
        ...
        Write-Host "  Found $($blocks.Count) FTDI/WinUSB-related driver store entry/entries:" -ForegroundColor DarkGray
        Write-Host ""
        ...
        if ($winusbEntries.Count -gt 0) {
            ...
            Write-Explain @"
WinUSB entries are generated by ftdi-unbind.exe (or Zadig) each time
a device is switched to WinUSB mode. They are normal if you have run
ftdi-unbind.exe before.

However, if ftdi-bind.exe reports 'RESTORE_ERR_DRIVERLESS' after
running (meaning the COM port does not return after binding), these
entries may be winning the driver-selection race over the FTDI VCP
driver. This is the 'stale oem*.inf' problem.
...
"@
            $script:Issues.Add("WinUSB/libwdi entries in driver store — may conflict with VCP driver if ftdi-bind fails")
